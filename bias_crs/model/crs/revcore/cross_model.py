from bias_crs.model.crs.revcore.transformer import _build_encoder, _build_decoder4kg
from .graph import SelfAttentionLayer,SelfAttentionLayer_batch
...
def _create_embeddings(dictionary, embedding_size, padding_idx):
    ...
    data_dir = "bias_crs/data/dataset/redial/revcore_data/" 
    ...
    e.weight.data.copy_(torch.from_numpy(np.load(data_dir + 'word2vec_redial.npy')))
...
def concept_edge_list4GCN():
    data_dir = "bias_crs/data/dataset/redial/revcore_data/"
    node2index=json.load(open(data_dir + 'key2index_3rd.json',encoding='utf-8'))
    f=open(data_dir + 'conceptnet_edges2nd.txt',encoding='utf-8')
    edges=set()
    stopwords=set([word.strip() for word in open(data_dir + 'stopwords.txt',encoding='utf-8')])
    for line in f:
...
class CrossModel(nn.Module):
    def __init__(self, opt, dictionary, is_finetune=False, padding_idx=0, start_idx=1, end_idx=2, longest_label=1):
        ...
        self.data_dir = "bias_crs/data/dataset/redial/revcore_data/"
        self.batch_size = opt['rec']['batch_size']
        ...
        self.embeddings = _create_embeddings(
            dictionary, opt['embedding_dim'], self.pad_idx
        )

        self.concept_embeddings=_create_entity_embeddings(
            opt['rec']['n_concept']+1, opt['rec']['dim'], 0)
        self.concept_padding = 0

        self.kg = pkl.load(open(self.data_dir + "subkg.pkl", "rb"))
        ...
        self.db_norm = nn.Linear(opt['rec']['dim'], opt['embedding_dim'])
        self.kg_norm = nn.Linear(opt['rec']['dim'], opt['embedding_dim'])

        self.db_attn_norm=nn.Linear(opt['rec']['dim'],opt['embedding_dim'])
        self.kg_attn_norm=nn.Linear(opt['rec']['dim'],opt['embedding_dim'])

        self.criterion = nn.CrossEntropyLoss(reduce=False)

        self.self_attn = SelfAttentionLayer_batch(opt['rec']['dim'], opt['rec']['dim'])
        self.self_attn_db = SelfAttentionLayer(opt['rec']['dim'], opt['rec']['dim'])
        ##################
        self.self_attn_rv = SelfAttentionLayer_batch(opt['embedding_dim'], opt['embedding_dim'])
        ##################

        self.user_norm = nn.Linear(opt['rec']['dim'] * 2, opt['rec']['dim'])
        self.gate_norm = nn.Linear(opt['rec']['dim'], 1)
        self.copy_norm = nn.Linear(opt['embedding_dim'] * 2 + opt['embedding_dim'], opt['embedding_dim'])
        ...
        self.info_con_norm = nn.Linear(opt['rec']['dim'], opt['rec']['dim'])
        self.info_db_norm = nn.Linear(opt['rec']['dim'], opt['rec']['dim'])
        self.info_output_db = nn.Linear(opt['rec']['dim'], opt['rec']['n_entity'])
        self.info_output_con = nn.Linear(opt['rec']['dim'], opt['rec']['n_concept']+1)
        ...
        self.user_representation_to_bias_1 = nn.Linear(opt['rec']['dim'], 512)
        self.user_representation_to_bias_2 = nn.Linear(512, len(dictionary) + 4)

        self.output_en = nn.Linear(opt['rec']['dim'], opt['rec']['n_entity'])

        self.embedding_size=opt['embedding_dim']
        self.dim=opt['rec']['dim']
        ...
        w2i=json.load(open(self.data_dir + 'word2index.json',encoding='utf-8'))
        self.i2w={w2i[word]:word for word in w2i}

        self.mask4key=torch.Tensor(np.load(self.data_dir + 'mask4key20rev.npy')).cuda()
        self.mask4movie=torch.Tensor(np.load(self.data_dir + 'mask4movie20rev.npy')).cuda()
    ...
    def forward(self, xs, ys, mask_ys, concept_mask, reviews_mask, seed_sets, labels, con_label, db_label, entity_vector, rec, test=True, prev_enc=None, maxlen=None,
                bsz=None):
        """
        ...
        gen_loss = None
        # generation ---------------------------------------
        rev_atten_emb, _ = self.self_attn_rv(reviews_states, reviews_mask.cuda())
