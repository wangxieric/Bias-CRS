...
from bias_crs.evaluator import conv
import random
...
class TGReDialDataLoader(BaseDataLoader):
    ...
    def __init__(self, subset=None, opt=None, dataset=None, vocab=None, side_data=None):
        """
    ...
    def rec_batchify(self, batch):
        ...
        for conv_dict in batch:
            ...
            if self.opt['model_name'] == 'C2CSR':
                context = self._get_original_context_for_rec(conv_dict['context_tokens'])
                ...
                input_ids, target_pos, input_mask, sample_negs = self._process_history(context_items, item_id)
            
            batch_context.append(context)

            if 'interaction_history' in conv_dict:
                context_items = conv_dict['interaction_history'] + conv_dict['context_items']
            else:
        ...
        if self.opt['model_name'] == 'C2CRS':
            batch = {
                ...
                'context_mask': (batch_context != 0).long(),
                'context_pad_mask': (batch_context == 0).long(),
    ...
    def conv_batchify_C2CRS(self, batch):
    ...
    def conv_batchify(self, batch):
        if self.opt['model_name'] == 'C2CRS':
            finish_batch = self.conv_batchify_C2CRS(batch)
            finish_batch = self.rec_batchify_pipeline_add_disInfo(finish_batch, batch)
            finish_batch = self.rec_batchify_pipeline_add_entities_mask(finish_batch, batch)
            return finish_batch
        else:
